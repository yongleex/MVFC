...
OutRatio = 0.15;% Outlier ratio
randn('state',100);    Vx = Vx + U_max*(sigma)*randn(size(Vx)).*x; % adding Gaussian noise
randn('state',150);    Vy = Vy + U_max*(sigma)*randn(size(Vx)).*x; % adding Gaussian noise

OutlierIndex_Truth = ones(size(Vx));
rand('state',200);    I = randperm(numel(Vx));
n = round(OutRatio*numel(Vx));% the outlier ratio =0.50
rand('state',200);    Vx(I(1:n)) =Vx(I(1:n))+ (1-0.0*x(I(1:n))).*(rand(1,n)-0.5)*4*U_max; % adding outliers
rand('state',300);    Vy(I(1:n)) =Vy(I(1:n))+ (1-0.0*x(I(1:n))).*(rand(1,n)-0.5)*4*U_max; % adding outliers
