function Flow4case1()
%% Visualized Displayment on Experimental Fields (Turbulent flow): Fig.12 in the paper
...
VecFld = MVFC(Vx,Vy,0.01);  %Our MMVFC Method
...
s5 = vssim(Vx_MVFC,Vy_MVFC,u0,v0);  e5 = nrmse(Vx_MVFC,Vy_MVFC,u0,v0);    % MVFC
...
figure;quiver(x,y,u,v);title('Original flow with outliers');
...
Fig12a = [x(:),y(:),x(:)+0.005*Vx(:),y(:)+0.005*Vy(:),OutlierIndex_Truth(:)];
Fig12b = [x(:),y(:),x(:)+0.005*Vx_CON(:),y(:)+0.005*Vy_CON(:),sqrt(Vx_CON(:).^2+Vy_CON(:).^2)];
Fig12c = [x(:),y(:),x(:)+0.005*Vx_DCT(:),y(:)+0.005*Vy_DCT(:),sqrt(Vx_DCT(:).^2+Vy_DCT(:).^2)];
Fig12d = [x(:),y(:),x(:)+0.005*Vx_VFC(:),y(:)+0.005*Vy_VFC(:),sqrt(Vx_VFC(:).^2+Vy_VFC(:).^2)];
