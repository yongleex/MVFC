...
Vy = -2*U_max*x.*y;
Omiga = 2*U_max*y;
Amp = U_max*x.*sqrt(x.^2+4*y.^2)+1;
u=Vx;v=Vy;

%%   Corrupt the original flow
sigma = 0.05;%standard deviation of the corrupted  Gaussiannoise, the value is 0.01*Vmax
OutRatio = 0.050;% Outlier ratio
randn('state',100);    Vx = Vx + U_max.*(sigma).*randn(size(Vx)); % adding Gaussian noise
randn('state',150);    Vy = Vy + 0.25*U_max.*(sigma).*randn(size(Vx)); % adding Gaussian noise

OutlierIndex_Truth = ones(size(Vx));
rand('state',200);    I = randperm(numel(Vx));
n = round(OutRatio*numel(Vx));% the outlier ratio =0.50
rand('state',200);    Vx(I(1:n)) = (rand(n,1)-0.5)*4*U_max; % adding outliers
rand('state',300);    Vy(I(1:n)) = (rand(n,1)-0.5)*4*U_max; % adding outliers
OutlierIndex_Truth(I(1:n)) = 0;

% A cluster of outliers
Vx(17:20,17:20) = U_max; Vy(17:20,17:20)=0.0*U_max; OutlierIndex_Truth(17:20,17:20) = 0;
...
VecFld = MVFC(Vx,Vy);  %Our MMVFC Method
...
%     每种方法结果的表示
% Original Flow 实线上三角 ,'-^','LineWidth',2,'Color',[0.0 0.0 1.0], 'MarkerSize',6, 'MarkerEdgeColor','k', 'MarkerFaceColor',[0.0 0.0 1.0]);%ori flow
% 噪声流动      虚线下三角 ,'--v','LineWidth',2,'Color',[0,0.45,0.74], 'MarkerSize',6, 'MarkerEdgeColor','k', 'MarkerFaceColor',[0,0.45,0.74]);%corrupted flow
% CON           虚线菱形   ,'--d','LineWidth',2,'Color',[0.85,0.33,0.1], 'MarkerSize',6, 'MarkerEdgeColor','k', 'MarkerFaceColor',[0.85,0.33,0.1]);%con1
%               虚线六边形 ,'--h','LineWidth',2,'Color',[0.93,0.69,0.13], 'MarkerSize',6, 'MarkerEdgeColor','k', 'MarkerFaceColor',[0.93,0.69,0.13]);%con2
% DCT-PLS       虚线正方形 ,'--s','LineWidth',2,'Color',[0.49,0.18,0.56], 'MarkerSize',6, 'MarkerEdgeColor','k', 'MarkerFaceColor',[0.49,0.18,0.56]);%DCT-PLS flow
% VFC           实线圆形   ,'-o','LineWidth',2,'Color',[0.47,0.67,0.19] , 'MarkerSize',8, 'MarkerEdgeColor','k', 'MarkerFaceColor',[0.47,0.67,0.19] );%Ours flow
% FADV          虚线+号    ,'--+','LineWidth',2,'Color',[0.3,0.75,0.93], 'MarkerSize',6, 'MarkerEdgeColor','k', 'MarkerFaceColor',[0.3,0.75,0.93]);%FADV
% VTM           虚线五边形 ,'--p','LineWidth',2,'Color',[0.64,0.08,0.18], 'MarkerSize',6, 'MarkerEdgeColor','k', 'MarkerFaceColor',[0.64,0.08,0.18]);%VTM
