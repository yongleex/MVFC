...
%-  Outlier   Ratio: 0-40%  cluster outlier (with size 6) + 0 missing data
%
%To assess the performance of the method respecting to clustered outliers percentage effect, The reference methods are introducted in the file Readme.txt
%- Yong Lee 2016.08.10
%- leeyong@hust.edu.cn
...
% parfor i=1:MonteNum
    for j=1:Ns
        ...
        ssim_con(i,j) = vssim(Vx_CON,Vy_CON,u,v);        ssim_con1(i,j) = vssim(Vx_CON1,Vy_CON1,u,v);
        ...
        nrmse_con(i,j) = nrmse(Vx_CON,Vy_CON,u,v);        nrmse_con1(i,j) = nrmse(Vx_CON1,Vy_CON1,u,v);
...
MEAN_NRMSE_CON1 = mean(nrmse_con1,1);    VAR_NRMSE_CON1 = var(nrmse_con1,1);          LB_NRMSE_CON1=min(nrmse_con1,1);    UB_NRMSE_CON1=max(nrmse_con1,1);
...
MEAN_SSIM_CON1 = mean(ssim_con1,1);      VAR_SSIM_CON1 = var(ssim_con1,1);            LB_SSIM_CON1=min(ssim_con1,1);      UB_SSIM_CON1=max(ssim_con1,1);
...
%% Draw the results
% NRMSE
H1=figure();hold on;box on;xlim([0 40]);ylim([-0.01 1.01]);%title('\fontsize{12}NRMSE');
...
%SSIM
H2=figure();hold on;box on;xlim([0 40]);ylim([-0.01 1.01]);%title('\fontsize{12}SSIM');
...
%UDC
H3=figure();hold on;box on;xlim([0 40]);set(gca,'yscale','log');%ylim([-0.01 1.01]);%title('\fontsize{12}NRMSE');
...
%ODC
H4=figure();hold on;box on;xlim([0 40]);set(gca,'yscale','log');%ylim([-0.01 1.01]);%title('\fontsize{12}SSIM');
...
%TWDC
H5=figure();hold on;box on;xlim([0 40]);set(gca,'yscale','log');%ylim([-0.01 1.01]);%title('\fontsize{12}SSIM');
...
%% Save the results
%     saveas(H1,'../figs/outlier_VortexNRMSE.fig')
%     saveas(H2,'../figs/outlier_VortexSSIM.fig')
%     save('../figs/outlier001.mat')
